...
var account1_mnemonic =
  "canal enact luggage spring similar zoo couple stomach shoe laptop middle wonder eager monitor weather number heavy skirt siren purity spell maze warfare ability ten";
...
export async function main(setStatus: (status: string) => void) {
  // Asset Creation:
  ...
  setStatus("Changing Asset Configuration");
  await ChangeAssetConfiguration(
    recoveredAccount2,
    recoveredAccount1,
    note,
    assetID,
  ...
  await OptIntoAsset(recoveredAccount3.addr, note, assetID);
  ...
  setStatus("Transfering Asset");
  await TransferAsset(
    recoveredAccount1,
    recoveredAccount3.addr,
    10,
    note,
    assetID
  );
  ...
  // setStatus("Destroying Asset");
  // await destroyAsset(recoveredAccount1, assetID);

  setStatus("Create Asset");
}
...
async function OptIntoAsset(
  ...
  const revocationTarget: string | undefined = undefined;
  // CloseReaminerTo is set to undefined as
  // we are not closing out an asset
  const closeRemainderTo: string | undefined = undefined;
  // We are sending 0 asset
  const amount = 0;
  ...
  const rawSignedTxn = opttxn.signTxn(recoveredAccount3.sk);
...
async function FreezeAsset(
  ...
  const rawSignedTxn = ftxn.signTxn(recoveredAccount2.sk);
...
async function RevokeAsset(
  ...
  const rawSignedTxn = rtxn.signTxn(recoveredAccount2.sk);
...
async function TransferAsset(
  ...
  const rawSignedTxn = xtxn.signTxn(recoveredAccount1.sk);
...
async function CreateAsset(
  ...
  let rawSignedTxn = txn.signTxn(issuer.sk);
  ...
  await printCreatedAsset(algodclient, issuer.addr, assetID);
  await printAssetHolding(algodclient, issuer.addr, assetID);
...
export async function CreateAssetSimple(
  ...
  setExplorerLink(`https://testnet.algoexplorer.io/asset/${assetId}`);

  return assetId;
}

async function ChangeAssetConfiguration(
  ...
  const rawSignedTxn = ctxn.signTxn(recoveredAccount2.sk);
...
async function destroyAsset(manager: algosdk.Account, assetID: number) {
  // Destroy and Asset:
  ...
  const rawSignedTxn = dtxn.signTxn(recoveredAccount1.sk);
